...
//
// SUMMARY:
// This module provides functions for SD card with SPI.
//
// REVISION HISTORY:
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
#define SD_NCR_TIMEOUT     (u16)20          //byte times before command response is expected (must be at least 8)
#define SD_NAC_TIMEOUT     (u32)0x40000     //SPI byte times we should wait when performing read operations (should be at least 100ms for SD cards)
#define SD_WRITE_TIMEOUT   (u32)0xA0000     //SPI byte times to wait before timing out when the media is performing a write operation (should be at least 250ms for SD cards).

// O-arm PCB SPI1 settings

#define SD_SPI_ChipSelect() SDCard_CS1_SetLow()
#define SD_SPI_ChipDeselect() SDCard_CS1_SetHigh()
...
#define SD_SPI_GetCardDetect() SDCard_CD1_GetValue()
...
struct SD_ASYNC_IO
{
    u16 wNumBytes;         //Number of bytes to attempt to read or write in the next call to MDD_SDSPI_AsyncReadTasks() or MDD_SDSPI_AsyncWriteTasks.  May be updated between calls to the handler.
    u32 dwBytesRemaining; //Should be initialized to the total number of u08s that you wish to read or write.  This value is allowed to be greater than a single block size of the media.
    u08* pBuffer;          //Pointer to where the read/written u08s should be copied to/from.  May be updated between calls to the handler function.
    u32 dwAddress;        //Starting block address to read or to write to on the media.  Should only get initialized, do not modify after that.
    u08 bStateVariable;    //State machine variable.  Should get initialized to ASYNC_READ_QUEUED or ASYNC_WRITE_QUEUED to start an operation.  After that, do not modify until the read or write is complete.
};
...
struct MEDIA_INFORMATION
{
    ...
    u16    sectorSize;              // The sector size of the target device.
    enum SD_STATE state;
    u32 finalLBA;
    u08 gSDMode;
};
...
//Constants
#define SD_WRITE_RESPONSE_TOKEN_MASK   0x1F  //Bit mask to AND with the write token response u08 from the media, to clear the don't care bits.
...
struct SD_COMMAND_TABLE_ENTRY
{
    ...
    u08             CRC;            // The CRC value for that command
 ...
 * Private Prototypes
 *****************************************************************************/
static SD_RESPONSE SD_SendCmd(u08 cmd, u32 address);
static u08 SD_SPI_AsyncWriteTasks(struct SD_ASYNC_IO* info);
static u08 SD_SPI_AsyncReadTasks(struct SD_ASYNC_IO* info);
...
bool SD_SPI_SectorRead(u32 sector_address, u08* buffer, u16 sector_count)
{
...
bool SD_SPI_SectorWrite(u32 sector_address, const u08* buffer, u16 sector_count)
{
    struct SD_ASYNC_IO info;
    u08 status;
    bool result = false;
    ...
    for(i=0; i<sector_count; i++)
    {
        ...
        info.wNumBytes = 1 << 9;  //Equivalent to multiply by 512;
        info.dwBytesRemaining = 1 << 9;  //Equivalent to multiply by 512
        info.pBuffer = (u08*)buffer + (i << 9); //Equivalent to multiply by 512
        info.dwAddress = sector_address + i;
 ...
 */
static void SD_SPI_DelayMilliseconds(u08 milliseconds)
{
//    u16 timeout = SD_SLOW_CLOCK_DELAY_1MS_MIN * milliseconds;

    SD_SPI_ChipDeselect();
    while(milliseconds--) // MISO signal data is showing - 30u
    //while(timeout--) // earlier not shown MISO signal data but now showing
    {
...
bool SD_SPI_MediaInitialize (void)
{
    ...
    timeout = 100;//0; //added 22-FEB-2023
    do
    {
    ...
    if(timeout == 0)
    {
        ...
        if(response.r1._byte != 0x01) //Check if card in idle state now.
        {
            ...
            SD_SPI_ChipDeselect();
            SD_SPI_close(); // breaking SPI2-SD card init here - Manju
            return false;
        }            
        else
        {
    ...
    if(((response.r7.bytewise.argument._returnVal & 0xFFF) == 0x1AA) && (!response.r7.bitwise.bits.ILLEGAL_CMD))
    {
        ...
        //u08 4 = normal R1 response u08, bytes 3-0 are = OCR register value.
    ...
    //-------------------------------------------------------------
    //READ_BL_LEN: CSD Structure v1 cards always support 512 u08
    ...
    //1024 or 2048).  However, even on these cards, 512 u08 partial reads
    //and 512 u08 write are required to be supported.
    ...
    if(CSDResponse[0] & 0xC0)	//Check CSD_STRUCTURE field for v2+ struct device
    {
        ...
        c_size = (((u32)CSDResponse[7] & 0x3F) << 16) | ((u16)CSDResponse[8] << 8) | CSDResponse[9];

        mediaInformation.finalLBA = ((u32)(c_size + 1) * (u16)(1024u)) - 1; //-1 on end is correction factor, since LBA = 0 is valid.
    }
    else //if(CSDResponse[0] & 0xC0)	//Check CSD_STRUCTURE field for v1 struct device
    {
        ...
        c_size = ((u32)CSDResponse[6] << 16) | ((u16)CSDResponse[7] << 8) | CSDResponse[8];	//Get the bytes in the correct positions
        c_size &= 0x0003FFC0;	//Clear all bits that aren't part of the C_SIZE
        c_size = c_size >> 6;	//Shift value down, so the 12-bit C_SIZE is properly right justified in the u32.

        //Extract the C_SIZE_MULT field from the response.  It is a 3-bit number in bit position 49:47.
        c_size_mult = ((u16)((CSDResponse[9] & 0x03) << 1)) | ((u16)((CSDResponse[10] & 0x80) >> 7));
        ...
        mediaInformation.finalLBA = ((u32)(c_size + 1) * (u16)((u16)1 << (c_size_mult + 2)) * block_len) - 1;	//-1 on end is correction factor, since LBA = 0 is valid.		
    }	
...
static u08 SD_SPI_AsyncReadTasks(struct SD_ASYNC_IO* info)
{
    u08 bData;
    SD_RESPONSE response;
    static u16 blockCounter;
    static u32 longTimeoutCounter;
...
static u08 SD_SPI_AsyncWriteTasks(struct SD_ASYNC_IO* info)
{
    static u08 data_byte;
    static u16 blockCounter;
    static u32 WriteTimeout;
    static u08 command;
    u32 preEraseBlockCount;
    SD_RESPONSE response;
    ...
    switch(info->bStateVariable)
    {
        ...
        case SD_ASYNC_WRITE_TRANSMIT_PACKET:
            ...
            //Update local copy of pointer and u08 count.  Application firmware
        ...
        case SD_ASYNC_WRITE_MEDIA_BUSY:
            ...
            {
                WriteTimeout--;
                (void)SD_SPI_exchangeByte(0xFF);  //Dummy read to gobble up a u08 (ex: to ensure we meet NBR timing parameter)
                ...
                if(data_byte != 0x00)
                {
                    ...
                    if(ioInfo.dwBytesRemaining == 0)
                    {
                        ...
                        if(command == SD_WRITE_MULTI_BLOCK)
                        {
                            //We finished sending all bytes of data.  Send the stop token u08.
                            (void)SD_SPI_exchangeByte (SD_TOKEN_STOP_TRANSMISSION);
                            
                            //After sending the stop transmission token, we need to
                            //gobble up one u08 before checking for media busy (0x00).
