...
#define UCP_SOH_30SEC_TIMER_PERIOD_MS          30000 

//**************************************************************************************************
// Local Variables
//**************************************************************************************************
u16 dayOfYear, prevDayOfYear;
i08 *ApndStr1, *fStr;
volatile bool _ucpSohPostInitFlag = false;

i08 writeDataArray[BUFFER_SIZE]="", readDataArray[BUFFER_SIZE]="";
...
{
    bool status = false;
    long Temp1 = 2256, RemCap = 870, FullCap = 952, soc = 89, soh = 78;
    ucpSccCanData.CurrentTemp1Value = Temp1++;//_sccBmsIndex4Data.Temp1;
            
    ucpSccCanData.RemCap = RemCap++;//_sccBmsIndex0Data.RemainingCapacity;

    ucpSccCanData.FullCap = FullCap++;//_sccBmsIndex1Data.FullCapacity;
...
// Parameters:  None
// Returns:     None
// Overview:    This is to calculate average Temp1 value using Temp1 at power 
//              down and power up
//****************************************************************************************
void calcAvgTempOfUcpOffState(void)
{    
    // Copy last RTC saved in SD card CSV file
    ucpRungAvgData.csvFileLastRtc = parseStringArray[RTC]; 
    
    // Copy Temp1
    ucpRungAvgData.LatestSDCTemp1 = parseStringArray[TEMP1];  
    
    // Read Temp1 from Sedecal CAN
    ucpSccCanData.CurrentTemp1Value = readSccTemp1Value();   
    
    // Get System RTC from APP (UI))
    getRtcFromApp();
    appRtcSecCounter = ucpRungAvgData.csvFileLastRtc + 500;
    // if System RTC is greater than last RTC saved in SD card
    if(appRtcSecCounter > ucpRungAvgData.csvFileLastRtc)
        // Then do average of them
